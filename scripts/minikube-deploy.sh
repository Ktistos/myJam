...
BACKEND_IMAGE="${REQUESTED_BACKEND_IMAGE:-${BACKEND_IMAGE:-${DEFAULT_BACKEND_IMAGE}}}"
FRONTEND_IMAGE="${REQUESTED_FRONTEND_IMAGE:-${FRONTEND_IMAGE:-${DEFAULT_FRONTEND_IMAGE}}}"
...
apply_backend_config() {
  kubectl_cmd -n "${APP_NAMESPACE}" create configmap jam-backend-config \
  ...
  kubectl_cmd -n "${APP_NAMESPACE}" create secret generic jam-backend-secrets \
...
apply_postgres_secret() {
  kubectl_cmd -n "${APP_NAMESPACE}" create secret generic jam-postgres-secrets \
    --from-literal=POSTGRES_PASSWORD="${POSTGRES_PASSWORD}" \
    --dry-run=client -o yaml | kubectl_cmd apply -f -
}

apply_minio_tenant_secret() {
  kubectl_cmd -n "${STORAGE_NAMESPACE}" create secret generic jam-minio-env-configuration \
...
kubectl_cmd apply -f "${ROOT_DIR}/k8s/minikube/namespaces.yaml"

echo "Installing/refreshing MinIO Operator ${MINIO_OPERATOR_REF}..."
kubectl_cmd apply -k "github.com/minio/operator?ref=${MINIO_OPERATOR_REF}"
kubectl_cmd -n "${OPERATOR_NAMESPACE}" scale deployment/minio-operator --replicas="${MINIO_OPERATOR_REPLICAS}"
kubectl_cmd -n "${OPERATOR_NAMESPACE}" rollout status deployment/minio-operator --timeout=5m
...
kubectl_cmd apply -f "${ROOT_DIR}/k8s/minikube/postgres.yaml"
kubectl_cmd apply -f "${ROOT_DIR}/k8s/minikube/redis.yaml"
kubectl_cmd -n "${APP_NAMESPACE}" rollout status statefulset/postgres --timeout=5m
kubectl_cmd -n "${APP_NAMESPACE}" rollout status deployment/redis --timeout=5m

echo "Deploying MinIO tenant..."
sed "s/storageClassName: minio-wffc/storageClassName: ${MINIO_STORAGE_CLASS_NAME}/" \
  "${ROOT_DIR}/k8s/minikube/minio-tenant.yaml" | kubectl_cmd apply -f -
until kubectl_cmd -n "${STORAGE_NAMESPACE}" get pod -l v1.min.io/tenant=jam-minio -o name | grep -q .; do
  sleep 2
done
kubectl_cmd -n "${STORAGE_NAMESPACE}" wait --for=condition=Ready pod -l v1.min.io/tenant=jam-minio --timeout=10m
render_ingress_manifest "${ROOT_DIR}/k8s/minikube/minio-ingress.yaml" | kubectl_cmd apply -f -

echo "Running MinIO bucket bootstrap job..."
kubectl_cmd -n "${STORAGE_NAMESPACE}" delete job jam-minio-public-bucket --ignore-not-found
kubectl_cmd apply -f "${ROOT_DIR}/k8s/minikube/minio-bucket-policy-job.yaml"
kubectl_cmd -n "${STORAGE_NAMESPACE}" wait --for=condition=complete job/jam-minio-public-bucket --timeout=5m

echo "Running database migration job..."
kubectl_cmd -n "${APP_NAMESPACE}" delete job jam-backend-migrate --ignore-not-found
render_backend_image_manifest "${ROOT_DIR}/k8s/minikube/backend-migrate-job.yaml" | kubectl_cmd apply -f -
kubectl_cmd -n "${APP_NAMESPACE}" wait --for=condition=complete job/jam-backend-migrate --timeout=5m

echo "Deploying backend and frontend..."
kubectl_cmd apply -f "${ROOT_DIR}/k8s/minikube/backend.yaml"
kubectl_cmd apply -f "${ROOT_DIR}/k8s/minikube/frontend.yaml"
kubectl_cmd -n "${APP_NAMESPACE}" set image deployment/jam-backend backend="${BACKEND_IMAGE}"
kubectl_cmd -n "${APP_NAMESPACE}" set image deployment/jam-frontend frontend="${FRONTEND_IMAGE}"
render_ingress_manifest "${ROOT_DIR}/k8s/minikube/app-ingress.yaml" | kubectl_cmd apply -f -
kubectl_cmd -n "${APP_NAMESPACE}" rollout restart deployment/jam-backend
kubectl_cmd -n "${APP_NAMESPACE}" rollout restart deployment/jam-frontend
kubectl_cmd -n "${APP_NAMESPACE}" rollout status deployment/jam-backend --timeout=5m
kubectl_cmd -n "${APP_NAMESPACE}" rollout status deployment/jam-frontend --timeout=5m
